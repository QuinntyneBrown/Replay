...
import { EntityPage, IPagableService, baseUrl } from '@core';
...
export class ToDoService implements IPagableService<ToDo> {
  ...
  public complete(options: { toDo: ToDo }): Observable<{ toDo: ToDo }> {
    return this._client.put<{ toDo: ToDo }>(`${this._baseUrl}api/toDo/complete`, { toDo: options.toDo });
  }
}
